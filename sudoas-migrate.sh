...
printf 'This script will install opendoas (or opendoas-retry), doas tooling, optionally devtools-doas and fudo-git.\n'
...
if [ "$WANT_RETRY" -eq 1 ]; then
    ...
    if ! run_as_user "$NORMAL_USER" "$AUR_HELPER -S --needed opendoas-retry"; then
        err "Failed to install opendoas-retry with $AUR_HELPER. Please install manually as $NORMAL_USER."
    fi
else
    printf 'Installing opendoas from official repos via pacman\n'
    if ! run_as_root pacman -S --needed opendoas; then
        err "pacman failed to install opendoas. Please install manually."
    fi
fi

printf 'Installing vidoas and doasedit-alternative from AUR via %s as user %s\n' "$AUR_HELPER" "$NORMAL_USER"
if ! run_as_user "$NORMAL_USER" "$AUR_HELPER -S --needed vidoas doasedit-alternative"; then
    err "Failed to install vidoas/doasedit-alternative via $AUR_HELPER. Please install them manually as $NORMAL_USER if desired."
fi

DOAS_CONF="/etc/doas.conf"
if [ ! -f "$DOAS_CONF" ]; then
    printf '\n/etc/doas.conf not found. Creating minimal template (owner: root).\n'
    run_as_root sh -c 'cat > /etc/doas.conf <<'EOF'
## Minimal doas.conf automatically generated by sudoas-migrate.sh
...
if command -v doas >/dev/null 2>&1; then
    ...
    printf 'Note: doas executable not found in PATH. If not installed yet, install it before testing.\n'
fi
...
if confirm_default_no "Do you want to install devtools patched for doas (devtools-doas from AUR)?"; then
    ...
    if ! run_as_user "$NORMAL_USER" "$AUR_HELPER -S --needed devtools-doas"; then
...
if confirm_default_no "Do you want to replace sudo on the system by installing fudo-git (AUR)?"; then
    ...
    if ! run_as_user "$NORMAL_USER" "$AUR_HELPER -S --needed fudo-git"; then
        err "Failed to install fudo-git via $AUR_HELPER. Please install manually as $NORMAL_USER."
    fi
    ...
    if echo "$RC_FILE" | grep -q 'config.fish' 2>/dev/null; then
        printf 'Detected fish shell for %s - installing fish-opendoas-git from AUR and adding fish-compatible functions.\n' "$NORMAL_USER"
        if ! run_as_user "$NORMAL_USER" "$AUR_HELPER -S --needed fish-opendoas-git"; then
            err "Failed to install fish-opendoas-git. Please install manually as $NORMAL_USER."
        fi
